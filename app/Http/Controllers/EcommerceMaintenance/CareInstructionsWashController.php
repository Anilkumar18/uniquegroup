...
class CareInstructionsWashController extends Controller
{
	...
	public function WashSymbolList()
	{
	    ...
	    $symbollist=DB::select('call sp_selectcareInstructions(1,'.$customerid.',0,0,1,0)');	  
		
		$symbolimagelist=DB::select('call sp_selectcareInstructions(2,'.$customerid.',0,0,1,1)');
	  
	   //return view('admin.symbol',compact('chainSessionData','symbollist','user','usertype','symbolimagelist','chains'));
	 return view('ecommercemaintenance.careinstructions_wash',compact('symbollist','user','usertype','symbolimagelist','customers'));
	
	}

	public function addWashSymbollist(Request $request)
	 ...
	 // echo "Insertions"; exit;
								...
								$symbol_update = DB::select('call sp_CRUDsymbol(3,'.$request->editID.','.$customerid.',"'.$request->suspendedCarePhrase.'","'.$request->descriptionEnglish.'","'.$request->descriptionFrench.'","'.$request->descriptionSpanish.'","'.$request->descriptionPolish.'","'.$request->descriptionChinese.'","'.$request->descriptionRomanian.'","'.$request->descriptionTurkish.'","'.$request->descriptionPortuguese.'","'.$request->descriptionRussian.'","'.$request->symbolType.'",'.$selectimageid.',0,0)');  
									...
									$request->session()->flash('success', 'Symbol updated successfully!');  
										 ...
										 else
										{
										 ...
										 return redirect(url(route('admin.drysymbollist')));
										 
										}	
								
									return redirect(url(route('admin.washsymbollist')));
	  ...
	  public function WashSymbolactivate(Request $request)
		{
	 ...
	 public function WashSymboldeActivate(Request $request)
     {
...
public function WashSymbol_Select(Request $request,$id)
	{
