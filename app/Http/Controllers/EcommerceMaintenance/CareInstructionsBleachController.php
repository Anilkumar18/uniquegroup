...
class CareInstructionsBleachController extends Controller
{
	...
	public function BleachSymbolList()
	{
	    ...
	    $symbollist=DB::select('call sp_selectcareInstructions(1,'.$customerid.',0,0,2,0)');	  
		
		$symbolimagelist=DB::select('call sp_selectcareInstructions(2,'.$customerid.',0,0,2,1)');
	  
	   //return view('admin.symbol',compact('chainSessionData','symbollist','user','usertype','symbolimagelist','chains'));
	 return view('ecommercemaintenance.careinstructions_bleach',compact('symbollist','user','usertype','symbolimagelist','customers'));
	
	}

	public function addBleachSymbollist(Request $request)
	 ...
	 // echo "Insertions"; exit;
								...
								$symbol_update = DB::select('call sp_CRUDsymbol(3,'.$request->editID.','.$customerid.',"'.$request->suspendedCarePhrase.'","'.$request->descriptionEnglish.'","'.$request->descriptionFrench.'","'.$request->descriptionSpanish.'","'.$request->descriptionPolish.'","'.$request->descriptionChinese.'","'.$request->descriptionRomanian.'","'.$request->descriptionTurkish.'","'.$request->descriptionPortuguese.'","'.$request->descriptionRussian.'","'.$request->symbolType.'",'.$selectimageid.',0,0)');  
									...
									$request->session()->flash('success', 'Symbol updated successfully!');  
										 ...
										 else
										{
										 ...
										 return redirect(url(route('admin.bleachsymbollist')));
	  ...
	  public function BleachSymbolactivate(Request $request)
		{
	 ...
	 public function BleachSymboldeActivate(Request $request)
     {
...
public function BleachSymbol_Select(Request $request,$id)
	{
